...
???info "Una mica d'ajuda"
    ...
    Per calcular els angles interiors d'una figura de *N* costats, podem utilitzar la següent fórmula:
    
    $$
    angles = \frac{180 N - 360}{N}
    $$
...
* `#!py goto(y, y=None)`: situa el punter a la posició *(x, y)*, sense modificar-ne l'angle.
