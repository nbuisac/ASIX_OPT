...
Per sortir de l'intèrpret, cal escriure `#!py exit()` o bé `#!py exit` o bé prémer la combinació de tecles <kbd>Control</kbd> + <kbd>Z</kbd>
...
* nombres enters ([`#!py int`][int]{target="_blank"}), 

* de coma flotant ([`#!py float`][float]{target="_blank"}) 

* i complexos ([`#!py complex`][complex]{target="_blank"}).

!!!note "Més tipus de dades"

    Les fraccions ([`#!py fractions.Fraction`][fractions]{target="_blank"}) i els decimals ([`#!py decimal.Decimal`][decimal]{target="_blank"}) també estan disponibles mitjançant la importació des de la biblioteca estàndard.
