...
Una [`list`][list] és una col·lecció **mutable** d'elements en seqüència. Com la majoria de col·leccions (vegeu els elements integrats [`tuple`][tuple], [`dict`][dict] i [`set`][set]), les llistes poden contenir referències a qualsevol tipus de dades (o múltiples), incloses altres llistes. Com qualsevol [seqüència][], es pot accedir als elements mitjançant un índex `base0` des de l'esquerra i un índex `base1` des de la dreta.
...
Les llistes s'implementen com a [matrius dinàmiques][], i s'utilitzen més sovint per emmagatzemar grups de dades similars (cadenes, números, conjunts, etc) de longitud desconeguda (el nombre d'entrades es pot expandir o reduir arbitràriament).
...
Afegir *al principi*, o afegir al mig d'una llista són operacions menys eficients ja que requereix desplaçar tots els elements per mantenir l'ordre. Per a una estructura de dades similar que admet una memòria eficient `append`/`pop` des d'ambdós costats, cal utilitzar [collections.deque][], que té aproximadament el mateix rendiment *O(1)* en qualsevol direcció.

Com que *les llistes són **mutables*** i poden contenir referències a objectes Python arbitraris, també ocupen més espai a la memòria que un [`array.array`][array] o una [`tuple`][tuple] (que és immutable) de la mateixa longitud. Tot i això, les *llistes* són una estructura de dades extremadament **flexible** i **útil** i molts mètodes i operacions integrades a Python produeixen llistes com a sortida.
...
dades_anidades = [
    {"peix": "platejat", "mono": "marró", "lloro": "gris"},
    ("peix", "mamífer", "ocell"),
    ['aigua', 'jungla', 'cel']
]
```
...
Es pot accedir als elements dins de les llistes (així com als elements d'altres tipus de seqüències, com ara `str` i `tuple`), mitjançant la notació de claudàtors `[]`. Els índexs poden ser de esquerra--> dreta (a partir de zero) o dreta--> esquerra (a partir de -1). Mirem la paraula **`PYTHON`**.
...
Les llistes proporcionen un **[iterador][]** i es poden recórrer en *bucle* de la mateixa manera que altres tipus de seqüències, utilitzant o 
...
!!!quote "Extret de [https://exercism.org/tracks/python/concepts/lists][]"
