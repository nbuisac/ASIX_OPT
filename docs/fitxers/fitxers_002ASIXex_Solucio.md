...
Resol aquests exercicis de fitxers amb llenguatge Python. Pots utilitzar, o no, la llibreria _csv_.
...
???example "Exemple de codi base"

    ```py
    import csv

    # Exemple de lectura filtrant per columna
    with open('accessos.csv', mode='r', encoding='utf-8') as fitxer:
        lector_csv = csv.reader(fitxer)
        next(lector_csv)  # Saltem la capçalera si en té
        
        comptador_fallides = 0
        for fila in lector_csv:
            data, usuari, ip, estat = fila
            if estat == "fallida":
                print(f"ALERTA: Intent fallit de l'usuari {usuari} des de {ip}")
                comptador_fallides += 1

    print(f"Total d'alertes: {comptador_fallides}")
    ```
